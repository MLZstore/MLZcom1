...
BackendReady = False
try:
    from MLZ_steam import detect_steam_install_path
    from MLZ_http import close_global_client
    from MLZ_api import APIManager
    from MLZ_manifest import MLZManifestManager
    from MLZ_steam import has_MLZ_for_app, list_MLZ_apps, get_game_install_path, search_config_file, update_config_token, check_xinput_dll, install_xinput_dll, install_xinput_from_zip, download_and_install_xinput
    from MLZ_config import VERSION
    from MLZ_fixes import (
    ...
    logger.error(f"MLZCommunity: CRITICAL IMPORT ERROR: {e}")
...
class Plugin:
    ...
    def _inject_webkit_files(self):
        ...
        try:
            ...
            if os.path.exists(js_file_path):
                ...
                logger.error(f"MLZCommunity: Bundle not found")
        except Exception as e:
            logger.error(f'MLZCommunity: Failed to inject: {e}')

    def _front_end_loaded(self):
        if BackendReady:
            logger.log(f"MLZCommunity: v{VERSION} ready")
        else:
            logger.error("MLZCommunity: Backend failed to load!")
    ...
    def _load(self):
        ...
        if not BackendReady:
            logger.error("MLZCommunity: Backend import failed. Plugin running in limited mode.")
            self.plugin_dir = GetPluginDir() # Still need this for GetLocale
            self._inject_webkit_files() 
            Millennium.ready()
            return

        logger.log(f"MLZCommunity: backend loading (v{VERSION})")

        # Apple updates
        try:
            update_msg = apply_pending_update()
            if update_msg:
                logger.log(f"MLZCommunity: {update_msg}")
                store_last_message(update_msg)
        except Exception as e:
            logger.warn(f"MLZCommunity: Failed to apply pending update: {e}")
        ...
        self.MLZmanifest_manager = MLZManifestManager(self.backend_path, self.api_manager)
        
        logger.log("MLZCommunity: backend initialized")
        ...
        try:
            ...
            logger.warn(f"MLZCommunity: Failed to start auto-update: {e}")
        
        Millennium.ready()
        logger.log("MLZCommunity: backend ready")

    def _unload(self):
        logger.log("Unloading MLZCommunity plugin")
        if BackendReady:
            close_global_client()
...
def hasMLZForApp(appid: int) -> str:
    if not BackendReady: return error_response("Backend not ready")
    try:
        exists = has_MLZ_for_app(appid)
        return success_response(exists=exists)
    except Exception as e:
        logger.error(f'hasMLZForApp failed for {appid}: {e}')
        return error_response(str(e))

def addViaMLZManifest(appid: int) -> str:
    if not BackendReady: return error_response("Backend not ready")
    try:
        if plugin and plugin.api_manager and plugin.MLZmanifest_manager:
            endpoints = plugin.api_manager.get_download_endpoints()
            result = plugin.MLZmanifest_manager.add_via_MLZ(appid, endpoints)
            return json_response(result)
        else:
             return error_response("Plugin manager not initialized")
    except Exception as e:
        logger.error(f'addViaMLZManifest failed for {appid}: {e}')
        return error_response(str(e))

def GetStatus(appid: int) -> str:
    if not BackendReady: return error_response("Backend not ready")
    try:
        if plugin and plugin.MLZmanifest_manager:
            result = plugin.MLZmanifest_manager.get_download_status(appid)
            return json_response(result)
...
def removeViaMLZManifest(appid: int) -> str:
    if not BackendReady: return error_response("Backend not ready")
    try:
        if plugin and plugin.MLZmanifest_manager:
            result = plugin.MLZmanifest_manager.remove_via_MLZ(appid)
            return json_response(result)
        return error_response("Manager not initialized")
    except Exception as e:
        logger.error(f'removeViaMLZManifest failed for {appid}: {e}')
        return error_response(str(e))
...
def GetLogoData() -> str:
    """Return base64 data URI for the bundled logo (assets/MLZ-logo.png)."""
    try:
        path = _get_asset_path('MLZ-logo.png')
...
def SearchGameInAllDrives(appid: int) -> str:
    if not BackendReady: return error_response("Backend not ready")
    try:
        from MLZ_fixes import SPECIAL_PATH_GAMES, _find_game_in_all_drives
        ...
        logger.log(f"MLZCommunity: Searching for '{game_folder}' in all drives...")
        
        found_path = _find_game_in_all_drives(game_folder, search_path, exclude_paths=["SteamLibrary"])
        
        if found_path:
            logger.log(f"MLZCommunity: Found game at {found_path}")
            return success_response(installPath=found_path, message=f"Found game at {found_path}")
        else:
            logger.log(f"MLZCommunity: Game not found in any drive")
...
def SearchConfigFile(appid: int, filename: str = "configs.user.ini") -> str:
    if not BackendReady: return error_response("Backend not ready")
    try:
...
def UpdateConfigToken(appid: int, token: str) -> str:
    ...
    try:
        ...
        search_result = search_config_file(game_path, "configs.user.ini")
        ...
        if updated_files:
            ...
            return error_response("Failed to update any files", errors=errors)

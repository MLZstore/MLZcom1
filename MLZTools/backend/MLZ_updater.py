...
GITHUB_OWNER = "MDQI1"
GITHUB_REPO = "MLZTools"
GITHUB_ASSET_NAME = "MLZTools.zip"
...
def _get_current_version() -> str:
    ...
    try:
        ...
        logger.warn(f"MLZUpdater: Failed to read current version: {e}")
    return "0.0.0"
...
def _read_json(path: str) -> Dict:
    ...
    try:
        ...
        logger.warn(f"MLZUpdater: Failed to read {path}: {e}")
    return {}
...
def _write_json(path: str, data: Dict) -> bool:
    ...
    try:
        ...
        logger.warn(f"MLZUpdater: Failed to write {path}: {e}")
        return False
...
def _fetch_github_latest() -> Dict[str, Any]:
    ...
    headers = {
        ...
        "User-Agent": "MLZTools-Updater/1.0"
    }
    ...
    try:
        ...
        logger.warn(f"MLZUpdater: GitHub API failed: {e}")
        return {}
...
def apply_pending_update() -> str:
    ...
    try:
        logger.log(f"MLZUpdater: Applying pending update from {pending_zip}")
        ...
        if version:
            return f"تم تحديث MLZTools إلى الإصدار {version}. يرجى إعادة تشغيل Steam."
        return "تم تحديث MLZTools. يرجى إعادة تشغيل Steam."
        
    except Exception as e:
        logger.error(f"MLZUpdater: Failed to apply pending update: {e}")
        return ""
...
def check_for_updates() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.log(f"MLZUpdater: Current={current_version}, Latest={latest_version}, HasUpdate={has_update}")
        ...
        logger.error(f"MLZUpdater: Check failed: {e}")
...
def check_for_update_once() -> str:
    ...
    try:
        ...
        if not _is_newer_version(latest_version, current_version):
            logger.log(f"MLZUpdater: Up-to-date (current {current_version}, latest {latest_version})")
            return ""
        ...
        if _extract_update(pending_zip, plugin_dir):
            try:
                ...
                pass
            logger.log("MLZUpdater: Update extracted; will take effect after restart")
            return f"تم تحديث MLZTools إلى الإصدار {latest_version}. يرجى إعادة تشغيل Steam."
        else:
            # حفظ للتطبيق لاحقاً
        ...
        logger.error(f"MLZUpdater: check_for_update_once failed: {e}")
        return ""
...
def _download_update(zip_url: str, target_path: str) -> bool:
    ...
    try:
        logger.log(f"MLZUpdater: Downloading from {zip_url}")
        with httpx.Client(timeout=120.0, follow_redirects=True) as client:
            with client.stream("GET", zip_url, headers={"User-Agent": "MLZTools-Updater/1.0"}) as response:
                response.raise_for_status()
                with open(target_path, "wb") as f:
                    for chunk in response.iter_bytes(chunk_size=8192):
                        if chunk:
                            f.write(chunk)
        logger.log("MLZUpdater: Download complete")
        return True
    except Exception as e:
        logger.error(f"MLZUpdater: Download failed: {e}")
        return False
...
def _extract_update(zip_path: str, plugin_dir: str) -> bool:
    """استخراج ملف التحديث"""
    try:
        logger.log(f"MLZUpdater: Extracting {zip_path} to {plugin_dir}")
        ...
        with zipfile.ZipFile(zip_path, 'r') as z:
            ...
            if len(root_folders) == 1:
                root_folder = list(root_folders)[0] + '/'
                logger.log(f"MLZUpdater: Extracting from root folder: {root_folder}")
        ...
        logger.log("MLZUpdater: Extraction complete")
        return True
        
    except Exception as e:
        logger.error(f"MLZUpdater: Extraction failed: {e}")
        return False
...
def _download_and_apply_thread(download_url: str):
    ...
    try:
        ...
        logger.log(f"MLZUpdater: Downloading from {download_url}")
        
        with httpx.Client(timeout=300.0, follow_redirects=True) as client:
            with client.stream("GET", download_url, headers={"User-Agent": "MLZTools-Updater/1.0"}) as response:
                response.raise_for_status()
        ...
        error_msg = str(e)
        logger.error(f"MLZUpdater: Update failed: {e}")
...
def _periodic_update_check_worker():
    ...
    while True:
        try:
            ...
            logger.log("MLZUpdater: Running periodic background check...")
            message = check_for_update_once()
            if message:
                store_last_message(message)
                logger.log(f"MLZUpdater: Periodic check found update: {message}")
        except Exception as e:
            logger.warn(f"MLZUpdater: Periodic check failed: {e}")
...
def _start_periodic_update_checks():
    ...
    if _update_check_thread is None or not _update_check_thread.is_alive():
        ...
        logger.log(f"MLZUpdater: Started periodic update check thread (every {hours} hours)")
...
def _start_initial_check_worker():
    ...
    try:
        ...
        if message:
            store_last_message(message)
            logger.log(f"MLZUpdater: Initial check found update: {message}")
        ...
        logger.warn(f"MLZUpdater: Initial check failed: {e}")
        try:
            _start_periodic_update_checks()
        except:
            pass
...
def restart_steam() -> bool:
    ...
    try:
        ...
        if not os.path.exists(script_path):
            logger.error(f"MLZUpdater: restart script not found: {script_path}")
            return False
        
        CREATE_NO_WINDOW = 0x08000000
        subprocess.Popen(["cmd", "/C", script_path], creationflags=CREATE_NO_WINDOW)
        logger.log("MLZUpdater: Restart script launched")
        return True
    except Exception as e:
        logger.error(f"MLZUpdater: Failed to restart Steam: {e}")
        return False
...
def check_for_updates_now() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.warn(f"MLZUpdater: CheckForUpdatesNow failed: {e}")

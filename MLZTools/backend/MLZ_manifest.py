...
import PluginUtils
from MLZ_http import get_global_client
from MLZ_steam import get_stplug_in_path
from MLZ_api import APIManager
from MLZ_config import HTTP_CHUNK_SIZE, DOWNLOAD_PROGRESS_UPDATE_INTERVAL
...
class MLZManifestManager:
    ...
    def _download_from_backend(self, appid: int, endpoint: str = "") -> None:
        try:
            ...
            logger.error(f"MLZCommunity: Fatal error in download setup: {e}")
            self._set_download_state(appid, {
                'status': 'failed',
        ...
        try:
            # MLZCommunity API endpoint
            download_url = f'https://generator.ryuu.lol/secure_download?appid={appid}&auth_code=RYUUMANIFESTu6n47w'

            logger.log(f"MLZCommunity: Requesting download for AppID {appid}")
            ...
            try:
                # MLZTools User-Agent with longer timeout
                custom_headers = {'User-Agent': 'MLZtools-v61-stplugin-hoe'}
                ...
                with httpx.Client(timeout=download_timeout, follow_redirects=True) as httpx_client:
                    with httpx_client.stream('GET', download_url, headers=custom_headers) as resp:
                        ...
                        try:
                            ...
                            logger.warn(f"MLZCommunity: Could not parse Content-Length header: {e}")
                            total = 0

                        content_type = resp.headers.get('content-type', '').lower()
                        if 'application/json' in content_type:
                            error_text = resp.read().decode('utf-8')
                            logger.error(f"MLZCommunity: Received JSON error response: {error_text}")
                        ...
                        with open(temp_zip_path, 'wb', buffering=HTTP_CHUNK_SIZE) as f:
                            for chunk in resp.iter_bytes(chunk_size=HTTP_CHUNK_SIZE):
                                ...
                                try:
                                    ...
                                    logger.warn(f"MLZCommunity: Could not get timestamp for download progress: {e}")
                                    now_ts = 0.0
                ...
                logger.log(f"MLZCommunity: Downloaded {bytes_read} bytes to {temp_zip_path}")

                try:
                    is_zip = zipfile.is_zipfile(temp_zip_path)
                except Exception as e:
                    logger.warn(f"MLZCommunity: Could not verify if file is ZIP for app {appid}: {e}")
                    is_zip = False
                ...
                if is_zip:
                    ...
                    try:
                        ...
                        try:
                            ...
                            logger.warn(f"MLZCommunity: Could not copy file for app {appid}: {e}")
                            raise
                        ...
                        logger.log(f"MLZCommunity: Installed single LUA file for app {appid}: {dest_file}")
                    except Exception as e:
                        logger.error(f"MLZCommunity: Failed to install non-zip payload for app {appid}: {e}")
                        raise

                self._set_download_state(appid, {
                    'status': 'done',
                    'success': True,
                    'api': f'MLZCommunity ({endpoint})'
                })
                ...
                if os.path.exists(temp_zip_path):
                    try:
                        ...
                        logger.warn(f"MLZCommunity: Could not remove temp file on error cleanup for app {appid}: {e2}")
            ...
            logger.error(f"MLZCommunity: Backend download failed: {str(e)}")
            self._set_download_state(appid, {
                'status': 'failed',
    ...
    def _extract_and_add_lua_from_zip(self, appid: int, zip_path: str, endpoint: str) -> None:
        try:
            ...
            logger.log(f"MLZCommunity: Extracting ZIP file {zip_path} to {target_dir}")

            with zipfile.ZipFile(zip_path, 'r') as zip_file:
                file_list = zip_file.namelist()
                logger.log(f"MLZCommunity: ZIP contains {len(file_list)} files")

                lua_files = [f for f in file_list if f.lower().endswith('.lua')]

                if not lua_files:
                    logger.warn(f"MLZCommunity: No .lua files found in ZIP, extracting all files")
                    lua_files = file_list
                ...
                for file_name in lua_files:
                    ...
                    try:
                        ...
                        logger.error(f"MLZCommunity: Failed to extract {file_name}: {e}")
                        continue

            if not installed_files:
                raise Exception("No files were successfully extracted from ZIP")

            logger.log(f"MLZCommunity: Successfully installed {len(installed_files)} files from {endpoint}")
            self._set_download_state(appid, {
            ...
            logger.error(f'MLZCommunity: Invalid ZIP file for app {appid}: {e}')
            raise Exception(f"Invalid ZIP file: {str(e)}")
        except Exception as e:
            logger.error(f'MLZCommunity: Failed to extract ZIP for app {appid}: {e}')
            raise

    def add_via_MLZ(self, appid: int, endpoints: Optional[List[str]] = None) -> Dict[str, Any]:
        try:
            appid = int(appid)
        ...
        def safe_availability_check_wrapper(appid, endpoints_to_check):
            try:
                ...
                logger.error(f"MLZCommunity: Unhandled error in availability check thread: {e}")
                self._set_download_state(appid, {
                    'status': 'failed',
    ...
    def remove_via_MLZ(self, appid: int) -> Dict[str, Any]:
        try:
            appid = int(appid)
        except (ValueError, TypeError):
            return {'success': False, 'error': 'Invalid appid'}
        ...
        try:
            ...
            for filename in os.listdir(stplug_path):
                if filename.startswith(f'{appid}_') and filename.endswith('.manifest'):
                    ...
                    logger.log(f"MLZCommunity: Removed {manifest_file}")

            if removed_files:
                logger.log(f"MLZCommunity: Successfully removed {len(removed_files)} files for app {appid}: {removed_files}")
            ...
            logger.error(f"MLZCommunity: Error removing files for app {appid}: {e}")
